...
现有功能�?做日常任务任务，每日抽奖（有机会活动京豆，使用的是免费机会，不消耗WO币）
自动使用WO币购买装饰品可以获得京豆，分别可获得5,20�?0,100,200,400,700�?200京豆�?
注：目前使用此脚本会给脚本内置的两个码进行助力，请知�?
活动入口：京东APP我的-游戏与更�?东东小窝
�?京东APP首页-搜索 玩一�?DIY理想�?微信小程序入口：
来客有礼 - > 首页 -> 东东小窝
网页入口（注：进入后不能再此刷新，否则会有问题，需重新输入此链接进入）
https://h5.m.jd.com/babelDiy/Zeus/2HFSytEAN99VPmMGZ6V4EYWus1x/index.html

已支持IOS双京东账�?Node.js支持N个京东账�?脚本兼容: QuantumultX, Surge, Loon, 小火箭，JSBox, Node.js
===============Quantumultx===============
[task_local]
#东东小窝
...
let isPurchaseShops = false;//是否一键加购商品到购物车，默认不加�?$.helpToken = [];
if ($.isNode()) {
...
async function doAllTask() {
  ...
  for (let item of $.taskList) {
    if (item.ssjjTaskInfo.type === 1) {
      //邀请好友助力自�?      $.createAssistUserID = item.ssjjTaskInfo.id;
    ...
    // 限时连连�?    if (item.ssjjTaskInfo.type === 3) {
    ...
    if (item.ssjjTaskInfo.type === 4) {
      ...
      for (let i = 0; i < new Array(item.ssjjTaskInfo.awardOfDayNum).fill('').length; i++) {
        await followShops('followShops', item.ssjjTaskInfo.id);//一键关注店�?        await queryDoneTaskRecord(item.ssjjTaskInfo.id, item.ssjjTaskInfo.type);
      }
    }
    ...
    if (item.ssjjTaskInfo.type === 6) {
      //关注4个频�?      if (item.doneNum === item.ssjjTaskInfo.awardOfDayNum) {
        console.log(`${item.ssjjTaskInfo.name}已完成[${item.doneNum}/${item.ssjjTaskInfo.awardOfDayNum}]`)
        continue
      }
    ...
    if (item.ssjjTaskInfo.type === 7) {
      //浏览3个频�?      if (item.doneNum === item.ssjjTaskInfo.awardOfDayNum) {
        console.log(`${item.ssjjTaskInfo.name}已完成[${item.doneNum}/${item.ssjjTaskInfo.awardOfDayNum}]`)
        continue
      }
      for (let i = 0; i < new Array(item.ssjjTaskInfo.awardOfDayNum || 1).fill('').length; i++) {
    ...
    if (isPurchaseShops && item.ssjjTaskInfo.type === 9) {
      ...
      for (let i = 0; i < new Array(item.ssjjTaskInfo.awardOfDayNum).fill('').length; i++) {
        await followShops('purchaseCommodities', item.ssjjTaskInfo.id);//一键加购商�?        await queryDoneTaskRecord(item.ssjjTaskInfo.id, item.ssjjTaskInfo.type);
      }
    }
...
//装饰领京�?function furnituresCenterPurchase(id, jdBeanNum) {
  return new Promise(resolve => {
