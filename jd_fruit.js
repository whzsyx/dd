...
已支持IOS双京东账�?Node.js支持N个京东账�?脚本兼容: QuantumultX, Surge, Loon, JSBox, Node.js
互助码shareCode请先手动运行脚本查看打印可看�?一天只能帮�?个人。多出的助力码无�?==========================Quantumultx=========================
[task_local]
#jd免费水果
...
//助力好友分享�?最�?�?否则后面的助力失�?,原因:京东农场每人每天只有3次助力机�?//此此内容是IOS用户下载脚本到本地使用，填写互助码的地方，同一京东账号的好友互助码请使用@符号隔开�?//下面给出两个账号的填写示例（iOS只支�?个京东账号）
let shareCodes = [ // 这个列表填入你要助力的好友的shareCode
 ''
]
let message = '', subTitle = '', option = {}, isFruitFinished = false;
const retainWater = 100;//保留水滴大于多少g,默认100g;
let jdNotify = false;//是否关闭通知，false打开通知推送，true关闭通知推�?let jdFruitBeanCard = false;//农场使用水滴换豆�?如果出现限时活动�?00g水换20�?此时比浇水划�?推荐换豆),true表示换豆(不浇�?,false表示不换�?继续浇水),脚本默认是浇�?let randomCount = $.isNode() ? 0 : 0;
...
async function jdFruit() {
  ...
  try {
    ...
    if ($.farmInfo.farmUserPro) {
      ...
      console.log(`\n【京东账�?{$.index}�?{$.UserName}）的${$.name}好友互助码�?{$.farmInfo.farmUserPro.shareCode}\n`);
      ...
      if ($.farmInfo.treeState === 2 || $.farmInfo.treeState === 3) {
        ...
        //已下单购�? 但未开始种植新的水�?        option['open-url'] = urlSchema;
...
async function doDailyTask() {
  ...
  // 被水滴砸�?  console.log(`被水滴砸中： ${$.farmInfo.todayGotWaterGoalTask.canPop ? '�? : '�?}`);
  ...
  if (!$.farmTask.gotBrowseTaskAdInit.f) {
    ...
    let browseSuccess = 0
    let browseFail = 0
    for (let advert of adverts) { //开始浏览广�?      if (advert.limit <= advert.hadFinishedTimes) {
  ...
  //给好友浇�?  if (!$.farmTask.waterFriendTaskInit.f) {
  ...
  // await Promise.all([
  //   clockInIn(),//打卡领水
  //   executeWaterRains(),//水滴�?  //   masterHelpShare(),//助力好友
  //   getExtraAward(),//领取额外水滴奖励
  //   turntableFarm()//天天抽奖得好�?  // ])
  await getAwardInviteFriend();
  await clockInIn();//打卡领水
  await executeWaterRains();//水滴�?  await getExtraAward();//领取额外水滴奖励
  await turntableFarm()//天天抽奖得好�?}
async function predictionFruit() {
  console.log('开始预测水果成熟时间\n');
  await initForFarm();
  await taskInitForFarm();
  let waterEveryDayT = $.farmTask.totalWaterTaskInit.totalWaterTaskTimes;//今天到到目前为止，浇了多少次�?  message += `【今日共浇水�?{waterEveryDayT}次\n`;
    ...
    for (; waterCount < $.farmTask.totalWaterTaskInit.totalWaterTaskLimit - $.farmTask.totalWaterTaskInit.totalWaterTaskTimes; waterCount++) {
      ...
      if ($.waterResult.code === '0') {
        ...
        if ($.waterResult.finished) {
          // 已证实，waterResult.finished为true，表示水果可以去领取兑换�?          isFruitFinished = true;
          break
        } else {
...
async function getTenWaterAward() {
  //领取10次浇水奖�?  if (!$.farmTask.totalWaterTaskInit.f && $.farmTask.totalWaterTaskInit.totalWaterTaskTimes >= $.farmTask.totalWaterTaskInit.totalWaterTaskLimit) {
...
async function doTenWaterAgain() {
  ...
  if (signCard > 0) {
    //使用加签�?    for (let i = 0; i < new Array(signCard).fill('').length; i++) {
  ...
  if (`${jdFruitBeanCard}` === 'true' && JSON.stringify($.myCardInfoRes).match('限时翻�?)) {
    ...
    if (totalEnergy >= 100 && $.myCardInfoRes.beanCard > 0) {
      //使用水滴换豆�?      await userMyCardForFarm('beanCard');
  ...
  //   totalEnergy  = $.farmInfo.farmUserPro.totalEnergy;
  // }
  // 所有的浇水(10次浇�?任务，获取水滴任务完成后，如果剩余水滴大于等�?0g,则继续浇�?保留部分水滴是用于完成第二天的浇�?0次的任务)
...
//领取阶段性水滴奖�?function gotStageAward() {
...
async function turntableFarm() {
  ...
  if ($.initForTurntableFarmRes.code === '0') {
    //领取定时奖励 //4小时一�?    let {timingIntervalHours, timingLastSysTime, sysTime, timingGotStatus, remainLotteryTimes, turntableInfos} = $.initForTurntableFarmRes;
    ...
    if (remainLotteryTimes > 0) {
      ...
      for (let i = 0; i < new Array(remainLotteryTimes).fill('').length; i++) {
        ...
        if ($.lotteryRes.code === '0') {
          ...
          //没有次数�?          if ($.lotteryRes.remainLotteryTimes === 0) {
            break
          }
        }
      }
...
async function getExtraAward() {
  ...
  if ($.masterHelpResult.code === '0') {
    if ($.masterHelpResult.masterHelpPeoples && $.masterHelpResult.masterHelpPeoples.length >= 5) {
      // 已有五人助力。领取助力后的奖�?      if (!$.masterHelpResult.masterGotFinal) {
        ...
        console.log("已经领取�?好友助力额外奖励");
        message += `【额外奖励】已被领取过\n`;
      }
    } else {
      console.log("助力好友未达�?�?);
      message += `【额外奖励】领取失�?原因：给您助力的人未�?个\n`;
    }
...
async function masterHelpShare() {
  ...
  let remainTimes = 3;//今日剩余助力次数,默认3次（京东农场每人每天3次助力机会）�?  let helpSuccessPeoples = '';//成功助力好友
...
//水滴�?async function executeWaterRains() {
  let executeWaterRain = !$.farmTask.waterRainInit.f;
  if (executeWaterRain) {
    console.log(`水滴雨任务，每天两次，最多可�?0g水滴`);
...
async function clockInIn() {
  ...
  if ($.clockInInit.code === '0') {
    // 签到得水�?    if (!$.clockInInit.todaySigned) {
      console.log('开始今日签�?);
    ...
    // 限时关注得水�?    if ($.clockInInit.themes && $.clockInInit.themes.length > 0) {
    ...
    // 限时领券得水�?    if ($.clockInInit.venderCoupons && $.clockInInit.venderCoupons.length > 0) {
...
//给好友浇�?async function doFriendsWater() {
...
//领取�?个好友浇水后的奖励水�?async function getWaterFriendGotAward() {
  await taskInitForFarm();
...
//接收成为对方好友的邀�?async function receiveFriendInvite() {
  for (let code of newShareCodes) {
    if (code === $.farmInfo.farmUserPro.shareCode) {
    ...
    // console.log(`接收邀请成为好友结�?${JSON.stringify($.inviteFriendRes)}`)
  ...
  // console.log(`开始接�?fbd26cc27ac44d6a7fed34092453f77的邀请\n`)
...
function requireConfig() {
  return new Promise(resolve => {
    ...
    if ($.isNode()) {
      ...
      console.log(`\nBoxJs设置�?{$.name}好友邀请码:${$.getdata('jd_fruit_inviter') ? $.getdata('jd_fruit_inviter') : '暂无'}\n`);
    }
